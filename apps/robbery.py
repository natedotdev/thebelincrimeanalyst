...
image_card = dbc.Card(
    [
        dbc.CardBody(
            [
                html.H4("Robbery Cases in Berlin", className="card-title", style={"padding":10}),
                dbc.CardImg(src="../assets/robbery2.jpg", title="Robbery"),
...
graph_card = dbc.Card(
    [
        dbc.CardBody(
            [
                html.H4("Robbery Cases in Berlin 2012-2019", className="card-title", style={"text-align": "center",
                                                                                       "color": "black"}),
                dbc.Button(
...
def update_graph_card(districts):
    if len(districts) == 0:
        ...
        df_filtered = df_filtered.groupby(["Year", "District"])[['Robbery']].median().reset_index()
        fig = px.line(df_filtered, x="Year", y="Robbery", color="District",
                      labels={"Robbery": "Robbery Cases (avg)"}).update_traces(mode='lines+markers')

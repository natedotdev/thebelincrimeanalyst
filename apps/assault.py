...
image_card = dbc.Card(
    [
        dbc.CardBody(
            [
                html.H4("Assault Cases in Berlin", className="card-title", style={"padding":10}),
                dbc.CardImg(src="../assets/assault3.png", title="Assault"),
                html.H6("Choose Berlin Districts:", className="card-text", style={"padding":10}),
                html.Div(id="the_alert_assault", children=[]),
...
graph_card = dbc.Card(
    [
        dbc.CardBody(
            [
                html.H4("Assault Cases in Berlin 2012-2019", className="card-title", style={"text-align": "center",
                                                                                       "color": "black"}),
                dbc.Button(
                ...
                dcc.Graph(id='line_chart_assault', figure={}),
                dcc.Dropdown(options=['line','Bar Plot', 'Scatter Plot'],
                        value='line',  # initial value displayed when page first loads
                        clearable=False),
            ],
        ),
    ],
...
@app.callback(
    Output("popover_assault", "is_open"),
    [Input("popover-bottom-target", "n_clicks")],
    [State("popover_assault", "is_open")],
)
...
@app.callback(
    [Output("line_chart_assault", "figure"),
     Output("the_alert_assault", "children")],
...
def update_graph_card(districts):
    if len(districts) == 0:
        ...
        df_filtered = df_filtered.groupby(["Year", "District"])[['Agg_assault']].median().reset_index()
        fig = px.line(df_filtered, x="Year", y="Agg_assault", color="District",
                      labels={"Agg_assault": "Assault Cases (avg)"}).update_traces(mode='lines+markers')
        return fig, dash.no_update

# Enable Graph changeability interact (For version 1.2)



@app.callback(
    Output("modal_assault", "is_open"),
    [Input("open", "n_clicks"), Input("close", "n_clicks")],
    [State("modal_assault", "is_open")],
)

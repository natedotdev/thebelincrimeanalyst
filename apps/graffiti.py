...
image_card = dbc.Card(
    [
        dbc.CardBody(
            [
                html.H4("Vandalism Cases in Berlin", className="card-title", style={"padding":10}),
                dbc.CardImg(src="../assets/graffiti2.jpeg", title="Vandalism"),
                html.H6("Choose Berlin Districts:", className="card-text", style={"padding":10}),
                html.Div(id="the_alert_graffiti", children=[]),
...
graph_card = dbc.Card(
    [
        dbc.CardBody(
            [
                html.H4("Vandalism Cases in Berlin 2012-2019", className="card-title", style={"text-align": "center",
                                                                                       "color": "black"}),
                dbc.Button(
                ...
                dcc.Graph(id='line_chart_graffiti', figure={}),
            ],
        ),
    ],
    className="h-100",
...
@app.callback(
    Output("popover_graffiti", "is_open"),
    [Input("popover-bottom-target", "n_clicks")],
    [State("popover_graffiti", "is_open")],
)
...
@app.callback(
    [Output("line_chart_graffiti", "figure"),
     Output("the_alert_graffiti", "children")],
...
def update_graph_card(districts):
    if len(districts) == 0:
        ...
        fig = px.line(df_filtered, x="Year", y="Graffiti", color="District",
                      labels={"Graffiti": "Vandalism Cases (avg)"}).update_traces(mode='lines+markers')
        return fig, dash.no_update

@app.callback(
    Output("modal_graffiti", "is_open"),
    [Input("open", "n_clicks"), Input("close", "n_clicks")],
    [State("modal_graffiti", "is_open")],
)
